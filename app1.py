...
def load_model():
    ...
    try:
        ...
        st.error("Error: 'hotel_clustering_model.pkl' not found. Please ensure 'train_model.py' was run and the model file is in the same directory.")
        st.stop()
...
if not all([scaler, kmeans, features]):
    st.error(" Model components (scaler, kmeans, features) are missing or incomplete. Cannot proceed.")
    st.stop()

st.set_page_config(layout="wide", page_title="🏨 Hotel Room Clustering App")
...
def generate_features(df):
    ...
    if missing_raw:
        st.error(
            f" Critical Error: The CSV file is missing required base columns. "
    ...
    try:
        ...
        st.error(f" Error during feature generation. Check data types in the base columns: {e}")
        return None
...
if uploaded_file:
    ...
    try:
        ...
        st.error(f" Failed to read CSV file. Please check file formatting or encoding: {e}")
        st.stop()
    ...
    if df_processed is not None:
        ...
        try:
            ...
            df_processed["Cluster_Name"] = df_processed["Cluster_ID"].map(name_map).fillna("Unknown")
             ...
             st.error(f" Error during scaling or prediction: {e}. Check for data type mismatches with the trained model.")
             st.stop()
        ...
        if pca is not None:
            try:
                ...
                st.warning(f" Could not generate PCA plot: {e}. Check PCA object integrity.")
...
if submit:
    ...
    if df_single is not None:
        ...
        try:
            ...
            st.error(f" Prediction failed: {e}. Please check the validity of numerical inputs.")
